      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <View style={styles.productInfoContainer}>
          ...
          <Text style={styles.priceText}>
            {item.currency || 'HKD'} {item.selling_price ? item.selling_price.toLocaleString() : '0'}
          </Text>
          
          {/* Seller Info */}
          <View style={styles.sellerContainer}>
            <Image 
              source={{ uri: item.lister_image || 'https://randomuser.me/api/portraits/women/44.jpg' }} 
              style={styles.sellerImage} 
            />
            <View style={styles.sellerInfo}>
              <Text style={styles.sellerName}>{item.lister_name || 'Unknown seller'}</Text>
              <Text style={styles.sellerId}>AC-{item.lister_id || '0000000000'}</Text>
              <Text style={styles.sellerLocation}>{item.lister_location || 'Location not specified'}</Text>
              <Text style={styles.sellerJoined}>Joined: {item.joined_date || 'January 2025'}</Text>
            </View>
          </View>
        </View>
          ...
          {expandedSections.specification && (
            <View style={styles.sectionContent}>
              ...
              <View style={styles.detailRow}>
                ...
                <Text style={styles.detailValue}>{item.model || 'Not specified'}</Text>
              </View>
              <View style={styles.detailRow}>
